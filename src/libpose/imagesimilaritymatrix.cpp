#include "imagesimilaritymatrix.h"
...
float ImageSimilarityMatrix::at(int row, int col)
{
    if(row>=imageSimilarityMatrix.rows)
    {
        cerr << "ISM contains " << imageSimilarityMatrix.rows << " rows, cannot request row " << row << end;
        return -1;
    }
    if(col>=imageSimilarityMatrix.cols)
    {
        cerr << "ISM contains " << imageSimilarityMatrix.cols << " cols, cannot request col " << col << end;
        return -1;
    }
    return imageSimilarityMatrix.at<float>(row,col);
}

bool ImageSimilarityMatrix::operator==(const ImageSimilarityMatrix &s)
{
...
{
    return !this->==(s); //return the opposite result to that generated by ==
}
...
bool ImageSimilarityMatrix::write(string filename)
{
...
//                cout<<"SPECIAL CASE" << endl;
            ...
            for(int x=0; x<maskMatOne.rows; ++x)
            {
                for(int y=0; y<maskMatOne.cols; ++y)
                {
                    ...
                    if(xTwo<imgTwo.width() && xTwo >=0 && yTwo < imgTwo.height() && yTwo >= 0 && !blackPixel)
                    {
                        mTwo = 1;
...
float ImageSimilarityMatrix::min() //find the non-zero minimum in the image similarity matrix
{
    float min = FLT_MAX;
    for(int i=0; i<imageSimilarityMatrix.rows; ++i)
    {
...
float ImageSimilarityMatrix::max() //find the non-zero minimum in the image similarity matrix
{
    float max = -1;
    for(int i=0; i<imageSimilarityMatrix.rows; ++i)
    {
        for(int j=0; j<projectImages.cols; ++j)
        {
            float val = imageSimilarityMatrix.at<float>(i,j);
            if(val>max)
                max=val;
...
float ImageSimilarityMatrix::mean() //find the non-zero minimum in the image similarity matrix
{
    float sum=0;
    float count=0;
    for(int i=0; i<imageSimilarityMatrix.rows; ++i)
...
float getPathCost(vector<int> path) //get cost for path through ISM
{
    ...
    {
        cost+=imageSimilarityMatrix.at(path[i-1], path[i]);
    }
    return cost;
}
