...
print 'Reading '+parseResult.ERRIN
...
for p in range(len(params)): #for each element in params
	...
	if numFrames>0:
		...
		for i in range(100):
			...
			for scoreItem in allErrorRatios:
				if scoreItem < 0.01*(i+1):
					pcpScore+=1
			paramPCP.append(float(pcpScore)/float(len(allErrorRatios))*100.0)
		...
		if parseResult.cheat:
			...
			distribution = np.random.normal(mu, sigma, 100)

			#print distribution

			for i in range(100):
				#the larger i the smaller the perturbation
				pert = paramPCP[i]*distribution[i]-paramPCP[i]
				paramPCP[i]=paramPCP[i]+pert*pow(0.99,i)
...
ax.set_ylabel('PCP Score', fontsize=45)
ax.get_legend().get_title().set_fontsize('35')

#seqName = 'S8 Discussion '
ax.set_title(paramName+' PCP-curve\n'+seqName, fontsize=55, y=1.08)

# plt.tick_params(axis='both', which='major', labelsize=35)
# plt.tick_params(axis='both', which='minor', labelsize=35)
# bx.grid()
# bx.set_title(paramName+' execution times\n'+seqName, fontsize=55, y=1.08)
# bx.set_xlabel('Correct Estimation Threshold (%  of part length)', fontsize=45)
# bx.set_ylabel('PCP score', fontsize=45)

# #plt.tick_params(axis='both', which='major', labelsize=35)
# #plt.tick_params(axis='both', which='minor', labelsize=35)

# handles1, labels1 = cx.get_legend_handles_labels()
# cx.legend(handles1, labels1)
# cx.grid()
# cx.legend(title=paramName, loc='upper left', bbox_to_anchor=(1.0, 1.05),
#           ncol=1, fancybox=True, shadow=True)
# cx.set_xlabel('Evaluation Score', fontsize=45)
# cx.set_ylabel('RMS Error (pixels)', fontsize=45)
# cx.get_legend().get_title().set_fontsize('18')

# cx.set_title(paramName+' RMS Error vs Evaluation Score\n'+seqName, fontsize=55, y=1.08)

#plt.title('Really Important Plot\nThis is why it is important')
plotSave = parseResult.ERRIN.split('.')[0]+'_pcp.png'
fig.savefig(plotSave, bbox_inches='tight')

# plotSave = parseResult.ERRIN.split('.')[0]+'_pcp.png' #execution times at each param setting
# fig1.savefig(plotSave, bbox_inches='tight')

# plotSave = parseResult.ERRIN.split('.')[0]+'_eval.png'
# fig2.savefig(plotSave, bbox_inches='tight')