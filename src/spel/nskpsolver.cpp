...
namespace SPEL
{
  ...
  std::vector<Solvlet> NSKPSolver::solve(Sequence& sequence, std::map<std::string, float>  params, const ImageSimilarityMatrix& ism) //inherited virtual
  {
    ...
    {
      auto sol = propagateKeyframes(propagatedFrames, params, ism, trees, ignore);
      ...
      {
        std::stringstream ss;
        ss << "Terminating keyframe propagation after " << iteration << " iterations.";
  ...
  std::vector<NSKPSolver::SolvletScore> NSKPSolver::propagateFrame(const int frameId, const std::vector<Frame*> &frames, std::map<std::string, float> params, const ImageSimilarityMatrix& ism, const std::vector<MinSpanningTree>& trees, std::vector<int>& ignore)
  {
    if (frames.size() == 0)
    {
  ...
  void NSKPSolver::emplaceDefaultParameters(std::map<std::string, float>& params) const noexcept
  {
    ...
    spelHelper::mergeParameters(params, COMMON_NSKP_SOLVER_PARAMETERS::getParameters());
  }
