...
int main (int argc, char **argv)
{
    ...
    //project loaded, now set up test parameters
    ...
    float jointCoeff_min=0.0, jointCoeff_max=1.0, jointCoeff_step=0.1,
            ...
            useCSdet_min=0.0, useCSdet_max=1.0, useCSdet_step=0.1,
            useHoGdet_min=0.0, useHoGdet_max=1.0, useHoGdet_step=0.1;
    ...
    for(float baseRotationRange = baseRotationRange_min; baseRotationRange<=baseRotationRange_max; baseRotationRange+=baseRotationRange_step)
    {
        for(float jointCoeff=jointCoeff_min; jointCoeff<=jointCoeff_max; jointCoeff+=jointCoeff_step)
        {
            ...
            {
                for(float useCSdet=useCSdet_min; useCSdet<=useCSdet_max; useCSdet+=useCSdet_step)
                {
                    for(float useHoGdet=useHoGdet_min; useHoGdet<=useHoGdet_max; useHoGdet+=useHoGdet_step)
                    {
                        ...
                        params.emplace("jointCoeff", jointCoeff); //set solver body part connectivity sensitivity
                        params.emplace("priorCoeff", 0.0); //set solver distance to prior sensitivity

                        params.emplace("useCSdet", useCSdet); //determine if ColHist detector is used and with what coefficient
                        params.emplace("useHoGdet", useHoGdet); //determine if HoG descriptor is used and with what coefficient
                        ...
                        params.emplace("partDepthRotationCoeff", partDepthRotationCoeff); //search radius increase for each depth level in the part tree
